...
public class ActivityDao {
	...
	public Boolean saveOrUpdate(Activity act) {
		...
		try {
			...
			String sql = null;
			if (act.getId() == null) {
				sql = "insert	into Activity(act_name,act_picture,sponsor,co_organizer,sign_up_start_date,sign_up_end_date, act_start_date,act_end_date,act_content,count_tmp) values(?,?,?,?,?,?,?,?,?,?)";
			} else {
			...
			System.out.println("neirong " + act.getActContent());
			ps.setString(9, act.getActContent());
			System.out.println(act.getActContent());
			if (act.getId() != null) {
				ps.setInt(10, act.getId());
			}else{
				System.out.println(123);
				ps.setString(10, act.getCount_tmp());
			}
	...
	private Activity resultSet2Activity(ResultSet rs) {
		...
		try {
			...
			act.setCount_tmp(rs.getString("count_tmp"));
			return act;
		} catch (SQLException e) {
			e.printStackTrace();
		}
