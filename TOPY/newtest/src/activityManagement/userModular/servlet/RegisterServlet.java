...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (user != null) {
			out.print("<script language=\"javascript\" >alert('user already exists');"
		...
		if (isSuccess) {
			out.print("<script language=\"javascript\" >alert('Registration successful and returning to the login screen');"
					+ " location.href = './login.jsp';</script>");
			out.flush();
			out.close();
		} else {
			out.print("<script language=\"javascript\" >alert('Bad registration');"
					+ " location.href = './register.jsp';</script>");
			out.flush();
			out.close();
		}
