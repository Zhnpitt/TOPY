...
public class UserEditServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (isSuccess) {
			...
			out.print("<script language=\"javascript\" >alert('revise successfully ');"
					+ " window.location.href = '/newtest/index.jsp';</script>");
			out.flush();
			out.close();

		} else {
			out.print("<script language=\"javascript\" >alert('fail to revise');" + " javascript:history.go(-1)</script>");
			out.flush();
			out.close();
		}
	}
